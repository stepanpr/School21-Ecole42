...
int cycle_NinjaTP(NinjaTrap& robot, std::string& enemy)
{
	...
	while (1)
	{
		...
		{																					//спосбоность		
			...
			FragTrap *frend1 = new FragTrap("FragTrap Paul");
			ScavTrap *frend2 = new ScavTrap("ScavTrap Raul");
			NinjaTrap *frend3 = new NinjaTrap("NinjaTrap Carmen");
			ClapTrap *frend4 = new NinjaTrap("ClapTrap Antonio");

			robot.ninjaShoebox(*frend1);
			robot.ninjaShoebox(*frend2);
			robot.ninjaShoebox(*frend3);
			robot.ninjaShoebox(*frend4);
...
int NinjaTP(std::string enemy)
{
	...
	<< "(1)\e[1;34m NoName " << std::setw(71) << "\e[0;34m(🏥: 60; 🔋:  120; 🛡: 0; 🏹:  5; 🥊: 60;)" << std::endl 	// (hit-points: 100; points of energy: 100; armor: 5)
